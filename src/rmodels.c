...
#if SUPPORT_GPU_SKINNING
        if (rlGetMaxVertexAttributes() >= MAX_MESH_VERTEX_BUFFERS)
        {
...
static void UpdateModelAnimationVertexBuffers(Model model)
{
    for (int m = 0; m < model.meshCount; m++)
    {
        ...
        // Skip if the shader is using the bones directly and we have assigned bones to a VBO
#if defined (SUPPORT_GPU_SKINNING)
        Material material = model.materials[model.meshMaterial[m]];
        if ((material.shader.locs[SHADER_LOC_VERTEX_BONEIDS] != -1) && mesh->vboId[RL_DEFAULT_SHADER_ATTRIB_LOCATION_BONEINDICES] != 0) continue;
#endif
